...
func auth(cfg *shared.Config, creds Credentials, name string, password []byte) (ok bool, a *AuthObject) {
	a = new(AuthObject)
	...
	if a.Admin && a.GID == shared.SuperGID {
		a.Super = true
	}
	return
}

func (s *SessionPool) New(cfg *shared.Config, name string, password []byte,
