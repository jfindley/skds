...
func TestAdminGroupAssign(t *testing.T) {
    ...
    adminPriv, err := crypto.Encrypt(groupKey.Priv, superKey, adminKey.Pub)
    if err != nil {
        t.Fatal(err)
    }
...
func TestAdminGroupNew(t *testing.T) {
    ...
    if group.Kind != "admin" || bytes.Compare(shared.HexDecode(group.PrivKey), key.Priv) != 0 || bytes.Compare(shared.HexDecode(group.PubKey), key.Pub) != 0 {
        t.Error("Group data does not match")
    }
    ...
    if group.Kind != "client" || bytes.Compare(shared.HexDecode(group.PrivKey), key.Priv) != 0 || bytes.Compare(shared.HexDecode(group.PubKey), key.Pub) != 0 {
        t.Error("Group data does not match")
    }
}
